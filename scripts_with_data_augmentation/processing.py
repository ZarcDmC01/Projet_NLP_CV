...
class ImageProcessingPipeline:
    ...
    # ─────────────────────────────────────────────
    # Construction des transforms (normal vs augmenté)
    # ─────────────────────────────────────────────

    def get_transform(self, augment=False, flip_prob=0.0,
                       rotation_degrees=8, color_jitter=0.15, translate=0.03):
        """
        Construit le pipeline de transformation torchvision utilisé avant
        passage dans le backbone CNN.

        :param augment: Si True, insère des transformations aléatoires
                        AVANT le resize/normalisation standard ImageNet.
                        Utilisé pour générer des versions augmentées des
                        images d'entraînement avant extraction de features
                        par un backbone gelé.
        :param flip_prob: Probabilité de flip horizontal. Mis à 0.0 par
                        défaut : sur Flickr8k, de nombreuses légendes
                        contiennent des indications spatiales ("man on the
                        right"...) qui deviennent fausses si l'image est
                        retournée mais pas la légende. À activer seulement
                        si vous savez que ça ne pose pas problème pour votre
                        cas d'usage.
        :param rotation_degrees: Amplitude max de rotation aléatoire.
        :param color_jitter: Amplitude du jitter de couleur (brightness/
                        contrast/saturation).
        :param translate: Amplitude de translation aléatoire (fraction de
                        l'image).

        Le backbone étant gelé, une augmentation trop agressive n'enseigne
        aucune invariance au modèle (contrairement à un CNN entraîné /
        fine-tuné avec augmentation) : elle ne fait qu'associer la même
        légende à un embedding différent. Des transformations modérées
        évitent de transformer l'augmentation en bruit d'étiquetage.
        """
        ...
        if augment:
            if flip_prob > 0:
                augmentation_layers.append(transforms.RandomHorizontalFlip(p=flip_prob))
            augmentation_layers += [
    ...
    def preprocess_single_image(self, image_path, augment=False):
        """
        Charge une image, l'ajuste, et applique la normalisation standard
        ImageNet requise par les backbones pré-entraînés (PyTorch).

        :param augment: Voir get_transform().
        """
        from PIL import Image

        img = Image.open(image_path).convert('RGB')
        preprocess = self.get_transform(augment=augment)
