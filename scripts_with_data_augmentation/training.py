...
class CaptionTrainingPipeline:
    ...
    def __init__(self, vocab_size, max_length, feature_dim=2048, original_feature_prob=0.5):
        """
        ...
        :param original_feature_prob: Probabilité de tirer la version ORIGINALE
                             (non augmentée) d'une image dans data_generator,
                             quand plusieurs versions sont disponibles. Le reste
                             de la probabilité est réparti également entre les
                             versions augmentées. Empêche les versions bruitées
                             de dominer le signal d'entraînement (par défaut,
                             l'original est vu 50% du temps, peu importe le
                             nombre de versions augmentées générées).
        """
        self.vocab_size = vocab_size
        self.max_length = max_length
        self.feature_dim = feature_dim
        self.original_feature_prob = original_feature_prob
        self.model = None
    ...
    def _sample_feature(self, feature_options):
        """
        Tire un vecteur de features parmi les versions disponibles pour une
        image (originale + éventuellement augmentées). La version
        originale (index 0, toujours celle produite par le transform non
        augmenté) est favorisée avec une probabilité `original_feature_prob` ;
        les versions augmentées se partagent équitablement le reste.
        """
        if len(feature_options) == 1:
            return feature_options[0]

        n_augmented = len(feature_options) - 1
        remaining_prob = 1.0 - self.original_feature_prob
        probs = [self.original_feature_prob] + [remaining_prob / n_augmented] * n_augmented
        idx = np.random.choice(len(feature_options), p=probs)
        return feature_options[idx]
    ...
    def data_generator(self, descriptions, image_features, tokenizer, batch_size):
        ...
        Note (data augmentation) : `image_features[key]` est une LISTE de
        vecteurs (une version originale + éventuellement des versions
        augmentées, voir main.py / ImageProcessingPipeline.get_transform).
        On tire une version à chaque légende rencontrée via `_sample_feature`
        (pondérée en faveur de l'originale), ce qui veut dire qu'à chaque
        epoch le modèle voit une combinaison différente image/légende, sans
        que le signal bruité ne domine. Si une seule version est disponible
        (ex : images de validation, non augmentées), le tirage retombe
        systématiquement sur l'unique vecteur existant.
        """
        ...
        while True:
            for key, desc_list in descriptions.items():
                ...
                feature_options = image_features[key]
                for desc in desc_list:
                    feature = self._sample_feature(feature_options)
